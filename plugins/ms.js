...
let baseURI = '/apps/' + config.HEROKU.APP_NAME;
   ...
   var l_dsc = ''
    ...
    Asena.addCommand({pattern: 'autosticker ?(.*)', fromMe: true, desc: Y_dsc, usage: '.sticker on / off' }, (async (message, match) => {
        if (match[1] == 'off') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['AUTO_STICKER']: 'false'
    ...
    Asena.addCommand({ pattern: 'caption ?(.*)', fromMe: true, desc: 'changes all captions', usage: '.caption *Made by AsenaMwol*' }, (async (message, match) => {
